execute if score count player_count matches 2.. if entity @a[distance=25..] run function owls:events/owl_delivery_options
execute if score count player_count matches 2.. if entity @a[distance=25..] run tag @s add sender
execute run tag @e[type=allay,tag=owl,sort=nearest,limit=1,tag=!active] add select
execute run data merge entity @e[type=allay,tag=owl,sort=nearest,limit=1,tag=!active] {NoAI:1b}
